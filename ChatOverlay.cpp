...
static DWORD g_LastMessageTime = 0;

static bool g_KeyState[256] = { false };
static ChatNicknameCallback_t g_NicknameCallback = nullptr;
...
static HHOOK g_hLLKeyboardHook = nullptr;
...
static void DrawTextSimple(const std::string& text, int x, int y, D3DCOLOR color);
...
static void DrawTextSimple(const std::string& text, int x, int y, D3DCOLOR color)
{
	...
	g_Font->DrawTextA(NULL, text.c_str(), -1, &rect, DT_NOCLIP, color);
}
...
void SendChatMessage(const std::string& msg)
{
	...
	g_ChatHistory.push_back({msg, GetTickCount()});
	g_LastMessageTime = GetTickCount();
	if (g_ChatHistory.size() > 6)
		g_ChatHistory.erase(g_ChatHistory.begin());
}

void ChatOverlay_SetNicknameCallback(ChatNicknameCallback_t cb)
{
	g_NicknameCallback = cb;
}

static void AddSystemMessage(const std::string& text)
{
	g_ChatHistory.push_back({text, GetTickCount()});
	g_LastMessageTime = GetTickCount();
	if (g_ChatHistory.size() > 6)
		g_ChatHistory.erase(g_ChatHistory.begin());
}

static void ProcessChatSend()
{
	...
	if (g_ChatBuffer[0] == '/')
	{
		std::string command = g_ChatBuffer;
		...
		if (command == "/name")
		{
			std::string name = (space != std::string::npos)
				? g_ChatBuffer.substr(space + 1) : "";
			if (!name.empty() && g_NicknameCallback)
				g_NicknameCallback(name);
		}
		else if (command == "/help")
		{
			AddSystemMessage("[System] Available commands:");
			AddSystemMessage("  /name <nickname>  - Change your nickname");
		}
		else
		{
	...
	{
		AddSystemMessage("[Chat] " + g_ChatBuffer);
	}
...
static HRESULT __stdcall hkEndScene(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if (!g_Font)
	{
		D3DXCreateFontA(pDevice, 18, 0, FW_BOLD, 1, FALSE,
			...
			DEFAULT_PITCH | FF_DONTCARE, "Consolas", &g_Font);
	}
	...
	int alpha = 255;
	if (!g_ChatOpen && g_LastMessageTime > 0)
	{
		DWORD elapsed = now - g_LastMessageTime;
		if (elapsed > 6000)
		{
	...
	int histSize = (int)g_ChatHistory.size();
	int showCount = g_ChatOpen ? histSize : (histSize < 6 ? histSize : 6);
	int startIdx = (int)g_ChatHistory.size() - showCount;
	D3DCOLOR color = D3DCOLOR_ARGB((BYTE)alpha, 255, 255, 255);
	for (int i = 0; i < showCount; i++)
	{
		DrawTextSimple(g_ChatHistory[startIdx + i].text, 20, startY + i * 20, color);
	}
	...
	if (g_ChatOpen)
	{
		...
		std::string text = "> " + g_ChatBuffer;
		if ((GetTickCount64() / 500) % 2)
			text += "_";

		DrawTextSimple(text, 25, 655,
...
{
	g_hLLKeyboardHook = SetWindowsHookEx(WH_KEYBOARD_LL, hkLowLevelKeyboard,
		moduleInstance, 0);
	if (!g_hLLKeyboardHook)
		return 1;
...
static DWORD WINAPI ChatInputThread(LPVOID)
{
	...
	while (true)
	{
		...
		if (!g_ChatOpen)
		{
			...
			bool slashWasPressed = g_KeyState[VK_OEM_2];
			...
			g_KeyState[VK_OEM_2] = slashPressed;
		}
		...
		if (g_ChatOpen)
		{
			for (int vk = VK_SPACE; vk <= VK_OEM_7; vk++)
			{
				...
				bool wasPressed = g_KeyState[vk];
				if (pressed && !wasPressed)
				{
					char c = MapVkToChar(vk, shift);
					if (c && g_ChatBuffer.size() < 128)
						g_ChatBuffer += c;
				}
				g_KeyState[vk] = pressed;
			}
			...
			// Backspace
			{
				...
				bool pressed = (state & 0x8000) != 0;
				bool wasPressed = g_KeyState[VK_BACK];
				if (pressed && !wasPressed && !g_ChatBuffer.empty())
					g_ChatBuffer.pop_back();
				g_KeyState[VK_BACK] = pressed;
			}
			...
			// Enter = send
			{
				...
				bool pressed = (state & 0x8000) != 0;
				bool wasPressed = g_KeyState[VK_RETURN];
				if (pressed && !wasPressed)
				{
					ProcessChatSend();
				}
				g_KeyState[VK_RETURN] = pressed;
			}
			...
			// Escape = cancel
			{
				...
				bool pressed = (state & 0x8000) != 0;
				bool wasPressed = g_KeyState[VK_ESCAPE];
				if (pressed && !wasPressed)
				{
					g_ChatBuffer.clear();
					g_ChatOpen = false;
				}
				g_KeyState[VK_ESCAPE] = pressed;
			}
		}
		else
		{
...
static LRESULT CALLBACK hkWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	if (g_ChatOpen)
	{
		...
		{
			char c = (char)wParam;
			if (c >= 32 && c <= 126 && g_ChatBuffer.size() < 128)
				g_ChatBuffer += c;
			return 0;
		}
...
{
	(void)hModule; // moduleInstance is extern from client.cpp

	g_GameWindow = FindMainWindow();

	g_OriginalWndProc = (WNDPROC)SetWindowLongPtr(
		g_GameWindow, GWLP_WNDPROC, (LONG_PTR)hkWndProc);
